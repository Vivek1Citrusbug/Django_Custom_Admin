(function () {
  ...
  let _svg, _zoomer, _currDoodle, _eraseState;

  Vue.component("doodle", {
    template:
      '<path :d="pathData" :stroke="dood.color" :stroke-width="dood.strokeWidth" :data-index="di" />',
    props: {
      dood: Object,
      di: Number,
    },
  ...
  new Vue({
    ...
    methods: {
      rotate() {
        ...
        _editorState.cleared.forEach(
          (doods) => (allDoods = allDoods.concat(doods))
        );
        allDoods.forEach((d) => {
          console.log(d);
      ...
      clear() {
        ...
        if (doods && doods.length) {
          ...
          _undoer.add({
            undo() {
              _svgState.doodles = _editorState.cleared.pop();
            },
            redo: doIt,
          });
        }
  ...
  function init() {
    ...
    function handleImage(result) {
      loadImg(result.url, function () {
        actuallyHandleImage(this, result.url, result.file);
      });
    }
    ...
    (function initDraw() {
      ...
      dragTracker({
        ...
        callbackDragStart: (_, pos) => {
          ...
          if (picker.domElement) {
            picker.closeHandler({ type: "mousedown" });
          }
        ...
        callbackDragEnd: (_, pos, start, cancelled) => {
          ...
          if (cancelledByClick && _editorState.erasing) {
            cancelled = false;
          }
  ...
  function doodleEnd(pos) {
    ...
    const ptsXY = curr.points.map((p) => ({ x: p[0], y: p[1] })),
      ptsSimpl = simplify(ptsXY, Math.sqrt(curr.strokeWidth / 2)).map((p) => [
        p.x,
        p.y,
      ]);
  ...
  function eraseStart(pos) {
    ...
    _undoer.add({
      undo() {
        state.erased.forEach((d) => (d.isErased = false));
      },
      redo() {
        state.erased.forEach((d) => (d.isErased = true));
      },
    });
    erasePoint(pos);
  }
  ...
  //Export JPG:
  function exportJPG(download = true) {
    _cl("export");
    ...
    function handleImg() {
      ...
      if (step0) {
        ...
        //..and export the JPG
        const quality = 0.9, //JPG at 90% quality
          name = _svgState.img.filename;
        canvas.toBlob(
          (b) => downloadBlob(b, name, download),
          "image/jpeg",
          quality
        );
        console.log(
    ...
    //The SVG's embedded source image doesn't transfer to 'img',
    //so we must paint the canvas in two steps
    loadImg(_svgState.img.url /*svgUrl*/, handleImg, download);
  ...
  function loadImg(url, callback, download) {
    const img = new Image();
  ...
  function downloadBlob(blob, filename, download = true) {
    _cl("download");
    ...
    if (download) {
      ...
      console.log($("#PE-image"));
      document.getElementById("PE-image").value = URL.createObjectURL(blob);
    }
  }
